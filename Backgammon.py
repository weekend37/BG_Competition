...
def play_a_game(commentary = False):
    ...
    while not game_over(board) and not check_for_error(board):
        ...
        for i in range(1+int(dice[0] == dice[1])):
            ...
            if player == 1:
#                func = tprAgent.action_test
#                move = func(board_copy,dice,player,i) 
                move = tprAgent.action_test(board_copy,dice,player,i) 
           ...
           # if not valid_move(move,board_copy,dice,player,i):
            #    sys.exit("Game forfeited. Player "+str(player)+" made an illegal move")
